...
class AdminNotificationFeed extends \Magento\AdminNotification\Model\Feed
{
    ...
    public function getFeedUrl()
    {
        ...
        $modulesParams = array();
        foreach($this->getMegnorModules() as $key => $module) {
            $key = str_replace('Megnor_', '', $key);
    ...
    /**
     * Get Megnor Modules Info
     *
     * @return $this
     */
    protected function getMegnorModules()
    {
        $modules = array();
        foreach($this->_moduleList->getAll() as $moduleName => $module) {
            if ( strpos($moduleName, 'Megnor_') !== false && $this->_moduleManager->isEnabled($moduleName) ) {
    ...
    {
        return $this->_cacheManager->load('megnor_admin_notifications_lastcheck');
    }
    ...
    {
        $this->_cacheManager->save(time(), 'megnor_admin_notifications_lastcheck');
        return $this;
    }
